...
sendAndLog loggerH botParams update =
  getSystemTime >>= sendMessage botParams update >>= L.hDebug loggerH . show
...
cycleLPProcessing loggerH botParams serverInfo =
  getLongPoll serverInfo >>= \lpr ->
    L.hDebug loggerH (show lpr) >>
    case lpr of
      LPRSuccess ts' updates' ->
        processUpdates loggerH botParams updates' >>= \newBotParams ->
          cycleLPProcessing loggerH newBotParams serverInfo {ts = ts'}
                -- https://vk.com/dev/bots_longpoll?f=2.2.%20%D0%9E%D1%88%D0%B8%D0%B1%D0%BA%D0%B8
                -- failed 1
      LPRFailureWithTS ts' ->
        cycleLPProcessing loggerH botParams serverInfo {ts = ts'}
                -- failed 2 and 3
      LPRFailure -> initLPProcessing loggerH botParams
...
processConfig (VkConfig token groupId helpMsg repeatMsg echoRepeatNumber) =
  let isInRange n = n > 0 && n < 6
      config =
        (token, groupId, helpMsg, repeatMsg, pack $ show echoRepeatNumber)
